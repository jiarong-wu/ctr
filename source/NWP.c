...
/**
   ...
   Following S. P. Carney, B. Engquist and R. D. Moser 2020,
   We are testing a Fringe layer. */

// Auxiliary forcing only depends on vertical position y
double forcing (double y, double total) {
  double forcing = 4.*total / sq(sq(L0-h_)) * sq(L0-h_ - 2*(y-h_)) * (5*(L0-h_) - 4*(y-h_));
  double half = (L0-h_)/2. + h_;
  double add = y > half ? 1. : 0.; // NOTICE: it's now hard-coded this boundary
  return add*forcing;
}
...
event acceleration (i++) {
  ...
  foreach_face(x)
    av.x[] += dPdx*(1.-f[]) + forcing(y, (ampl-dPdx)*(L0-h_))*(1.-f[]);
}
...
//    scalar fa[];
//    double ampl = sq(Ustar)/(L0-h_);
//    double dPdx = 0.5*ampl;
//    foreach () {
//     fa[] = forcing(y, (ampl-dPdx)*(L0-h_))*(1.-f[]);
//    }
//    ///////////////////
//    vertex scalar phi[]; 
//    foreach_vertex () 
//      phi[] = y; 
//    // default phi is y 
//    profiles ({u.x, u.y, u.z, uxuy, uxux, uyuy, uzuz, fa}, phi, rf = 0.5, fname = file, min = 0.8, max = 2.*pi); 
//  } 
...
event turbulence_stat (t += 0.5, last) {
  char filename[100];
  bool do_linear = true;
  bool print_bin = true;
  int Nslice = 256;
  ...
  // sprintf (filename, "./field/pair_2d_avg_t%g.bin", t); // pressure
  // output_2d_span_avg (filename,pair ,res, do_linear, print_bin);
  // sprintf (filename, "./field/ax_2d_avg_t%g.bin", t); // pressure
  // output_2d_span_avg (filename,accel.x ,res, do_linear, print_bin);
